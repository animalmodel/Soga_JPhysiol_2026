...
load([filePath,'data_Fig6_mean_firing_rate_72_neurons_catA.mat']);
load([filePath,'data_Fig6_regression_param_72_neurons_catA.mat']);
load([filePath,'data_Fig6_hip_and_knee_joint_catB.mat']);
load([filePath,'data_Fig6_mean_firing_rate_136_neurons_catB.mat']);
load([filePath,'data_Fig6_regression_param_136_neurons_catB.mat']);
...
Hip_dir_fig6  = zeros(nNeurons_catA,1);
Knee_dir_fig6 = zeros(nNeurons_catA,1);
...
for i = 1:nNeurons_catA
    ...
    Hip_dir_fig6(i)  = abs(mean_vec(1));
    Knee_dir_fig6(i)= abs(mean_vec(2));
end

%Compute r and theta
%Note that r is a unit vector and therefore equals 1
r = sqrt(Hip_dir_fig6.^2 + Knee_dir_fig6.^2);
theta = atan2(Knee_dir_fig6, Hip_dir_fig6);
...
Hip_dir_fig6  = zeros(nNeurons_catB,1);
Knee_dir_fig6 = zeros(nNeurons_catB,1);
...
fname = fullfile(figureFolder, 'Figure6.emf');
